...
document.addEventListener('DOMContentLoaded', () => {
    const btnThemePortal = document.getElementById('btn-theme-portal');
    
    // Verifica se o usuário já escolheu o tema claro nas Escalas
    if (localStorage.getItem('theme') === 'light') { 
        document.body.classList.add('light-mode'); 
        if(btnThemePortal) btnThemePortal.innerHTML = '🌙 Tema'; 
    }
    
    // Alterna o tema
    if(btnThemePortal) {
        btnThemePortal.addEventListener('click', () => {
            document.body.classList.toggle('light-mode');
            const isLight = document.body.classList.contains('light-mode');
            localStorage.setItem('theme', isLight ? 'light' : 'dark'); // Salva para as outras telas
            btnThemePortal.innerHTML = isLight ? '🌙 Tema' : '☀️ Tema';
        });
    }
});
